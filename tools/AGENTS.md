...
- `build-release.js`: ships exactly every root `*.lua`, the `*.toc`, and `README.md`, nested under a top-level `PIority/` folder so the zip extracts straight into `Interface/AddOns`. It reads the version from the `.toc` `## Version` line and names the zip `PIority-<version>.zip` — deliberately carrying no interface/client version, since one build serves every client listed in `## Interface` and client compatibility is upload metadata rather than part of the filename. Output goes to `builds/` unless `-o <dir>` is given. Source `pi_*.json` files are intentionally excluded (only their generated `.lua` ships).
