 ...
 * build-release.js
 * ----------------
 * Bundles the PIority addon into a distributable zip containing only the files
 * WoW needs to run it: every root *.lua, the *.toc, and README.md.
 *
 * Files are nested under a top-level "PIority/" folder inside the archive so the
 * zip extracts straight into Interface/AddOns the way WoW expects, and the zip
 * is named "PIority-<version>-<interface>.zip" using the ## Version and
 ...
 *
 * Usage:
 *   node tools/build-release.js            # writes PIority-<version>-<interface>.zip into builds/
